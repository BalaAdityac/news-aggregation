    ...
    <main className="flex-grow relative overflow-hidden bg-background-light dark:bg-black min-h-screen transition-colors duration-500">
      ...
      <div className="relative z-10 pt-16">
              ...
              {[...Array(2)].map((_, i) => (
                <div key={i} className="flex gap-6 px-3 items-stretch">
                  {articles.slice(0, 4).map(article => (
                    <div key={article.id} className="w-80 md:w-96 bg-white/60 dark:bg-slate-900/40 backdrop-blur-xl rounded-3xl p-6 shadow-xl dark:shadow-2xl shadow-nordic-accent/10 dark:shadow-black/20 border border-white/40 dark:border-white/10 flex flex-col gap-3 group hover:bg-white/80 dark:hover:bg-white/10 hover:border-red-500/40 dark:hover:border-red-500/40 hover:-translate-y-2 transition-all duration-500">
                      <span className="text-red-600 dark:text-red-400 font-black uppercase tracking-widest text-[10px] drop-shadow-sm dark:drop-shadow-md">{article.category}</span>
                      <h4 className="font-bold text-lg text-nordic-text dark:text-white group-hover:text-red-600 dark:group-hover:text-red-400 transition-colors line-clamp-2 drop-shadow-sm dark:drop-shadow-md">{article.title}</h4>
                      <p className="text-sm text-nordic-accent dark:text-white/80 line-clamp-4 leading-relaxed mt-2 drop-shadow-sm">{article.excerpt}</p>
                      <span className="text-[10px] text-nordic-accent/60 dark:text-white/50 mt-auto pt-4 block font-bold uppercase tracking-widest">{article.readTime}</span>
                    </div>
                  ))}
                </div>
              ))}
        ...
        <section className="py-24 px-6 relative z-10">
          <div className="bg-gradient-to-br from-white/80 via-white/60 to-white/80 dark:from-primary/20 dark:via-blue-900/20 dark:to-rose-500/20 backdrop-blur-3xl rounded-[3rem] p-12 max-w-4xl mx-auto text-center border border-white/50 dark:border-white/20 shadow-xl">
            <span className="material-symbols-outlined text-5xl text-primary mb-6 drop-shadow-sm">mail</span>
            <h3 className="text-4xl font-bold mb-4 text-nordic-text dark:text-white drop-shadow-sm">Subscribe to The Pulse</h3>
